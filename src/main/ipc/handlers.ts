...
const scanRegistry = new Map<number, { module: string; items: ScanItem[]; timestamp: number }>();
function registerScan(module: string, items: ScanItem[], scanId: number): void {
    const now = Date.now();
    for (const [id, scan] of scanRegistry.entries()) {
        if (now - scan.timestamp > 30 * 60 * 1000) {
            scanRegistry.delete(id);
        }
    }
    scanRegistry.set(scanId, { module, items: items.map(item => ({ ...item })), timestamp: now });
    while (scanRegistry.size > 10) {
        const oldest = scanRegistry.keys().next().value;
        if (oldest !== undefined) scanRegistry.delete(oldest);
    }
}
function validateCleanup(module: string, items: unknown, scanId: unknown): ScanItem[] {
    ...
    const verified: ScanItem[] = [];
    for (const item of items) {
        const original = valid.get(item.id);
        if (!original || original.path !== item.path || original.name !== item.name || original.category !== item.category) {
            throw new Error('Cleanup item was not produced by the requested scan.');
        }
        // Enforce verified metadata from original scan
        verified.push({
            ...item,
...
export function registerIpcHandlers() {
    ...
    ipcMain.handle(IPC_CHANNELS.DUPLICATE_ROLLBACK, async () => { try { await duplicateFinder.rollback(); return { success: true }; } catch (e) { return { error: String(e) }; } });
    ...
    ipcMain.handle(IPC_CHANNELS.STARTUP_ROLLBACK, async () => { try { await startupManager.rollback(); return { success: true }; } catch (e) { return { error: String(e) }; } });
