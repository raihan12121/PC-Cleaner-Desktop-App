...
const Registry: React.FC = () => {
    const { invoke: startScan, loading: scanning, error, data: scanData, setData: setScanData } = useIpc<any>(IPC_CHANNELS.REGISTRY_SCAN);
    ...
    const handleClean = async () => {
        if (!scanData) return;
        const selected = scanData.items.filter((i: any) => i.selected);
        ...
        if (confirm(`Clean ${selected.length} registry items? A backup will be created in the recovery folder.`)) {
            ...
            try {
                ...
                setScanData({
                    ...scanData,
                    items: scanData.items.filter((item: any) => !selected.find((s: any) => s.id === item.id)),
                });
            } catch (e) {
                alert('Registry cleanup failed: ' + e);
            } finally {
                setCleaning(false);
            }
        }
    ...
    const toggleItem = (id: string) => {
        ...
        setScanData({
            ...scanData,
            items: scanData.items.map((item: any) => item.id === id ? { ...item, selected: !item.selected } : item)
        });
    };
        ...
        <div className="p-8 h-full flex flex-col pt-12 overflow-y-auto">
            <div className="flex justify-between items-start mb-8">
                ...
                <div className="flex space-x-3">
                    ...
                    <button
                        onClick={handleClean}
                        disabled={!scanData || scanData.items.filter((i: any) => i.selected).length === 0 || cleaning}
                            ...
                            <div className="p-4 space-y-2">
                                {scanData.items.map((item: any) => (
                                    <div
                                        ...
                                        className={`p-4 rounded-xl border transition-all cursor-pointer flex items-center justify-between ${item.selected ? 'bg-blue-500/5 border-blue-500/30' : 'bg-slate-800/10 border-slate-800/50 hover:border-slate-700'}`}
                                    >
                                        ...
                                        <div className="text-[10px] font-bold text-slate-400 px-2 py-1 bg-slate-800 rounded uppercase">{item.category}</div>
                                    </div>
                                ))}
                            </div>
                        )}
